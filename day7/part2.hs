...
main = do
  ...
  print $ getSplits tachyonManifolds

type TachyonManifolds = Seq.Seq (Int, Seq.Seq Char)

type Coord = (Int, Int)

type Cache = Map.Map Coord Int
...
getSplits :: TachyonManifolds -> Int
...
getSplits' :: TachyonManifolds -> Coord -> Cache -> (Int, Cache)
getSplits' Seq.Empty _ cache = (1, cache)
getSplits' manifolds coord@(_, beamSourceCol) cache
 ...
 where
  newCache = Map.insert coord mergedSplits rCache
  mergedSplits = lTreeCount + rTreeCount
  (rTreeCount, rCache) = getSplits' manifoldRest rightSource lCache
  (lTreeCount, lCache) = getSplits' manifoldRest leftSource cache
