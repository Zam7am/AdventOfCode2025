...
module Data.Heap (
  ...
  uncons,
  viewMin, -- O(1)\/O(log n) :: Heap a -> Maybe (a, Heap a)
...
import Data.Data (Constr, Data (..), DataType, Fixity (Prefix), constrIndex, mkConstr, mkDataType)
...
partition p (Heap _ leq t) = F.foldMap f t
 where
  f x
    | p x = (singletonWith leq x, mempty)
